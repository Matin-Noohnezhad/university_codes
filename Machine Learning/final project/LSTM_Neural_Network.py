...
#
pad_train = pad_sequences(train['review'], maxlen=maxlen, padding='post', value=0)
pad_test = pad_sequences(test['review'], maxlen=maxlen, padding='post', value=0)
###################     neural network model with LSTM   ###################
model = Sequential()
# assume we have 32 features for each word in sentence
model.add(Embedding(vocab_size+1, 32, input_length=maxlen , dropout=0.2))
model.add(Bidirectional(LSTM(1, dropout=0.2, recurrent_dropout=0.2)))

model.add(Dense(1, activation='sigmoid'))

model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
model.fit(pad_train, np.array(train['sentiment']), batch_size=100, epochs=10, validation_split=0.2)
...
output.to_csv("LSTM_Neural_Network.csv", index=False, quoting=3)
################################
end = time.time()
print(end - start)#6658.3161787986755(111 minutes)
