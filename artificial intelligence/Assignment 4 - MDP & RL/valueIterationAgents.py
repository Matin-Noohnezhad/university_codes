...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for _ in range(iterations): # in python _ is can be a variable name just like a or b or jack.
            ...
            ----- # copy the values of state, don't change them live

            for ---- in ----:  # go through every state
    ...
    def computeQValueFromValues(self, state, action):
        ...
        q_value = 0
        for ------,------- in ------------(------, ------): # iterate over Q(s,a,s'),P(s,a,a')
            q_value += ----- * ( ----------- + --------*---------- ) # update q value
    ...
    def computeActionFromValues(self, state):
        ...
        actions = ----------- # get possible actions from this state. it is stored in mdp
        if # ----- action is empty: (it is terminal state, so there is no possible actions)
            return None
