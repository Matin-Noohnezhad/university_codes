...
//    public int[] eventTable = new int[4];
    public double[] eventTable;
    Observer observer;
    public double queueRange = 4;
    public static float lambda = 2;
    public static int delta = 3;
    public static int mio = 9;

    public Executer(int simulationTime, int noQueues, int noCPUs, float lambda, int delta, int mio) {
        this.lambda = lambda;
        this.delta = delta;
        this.mio = mio;
        this.queueRange = ((double)20/noQueues);
        config = new Config(noQueues, noCPUs);// 2 entities 2 queues 2 servers
        observer = new Observer(config);
        this.simulationTime = simulationTime;
        clock = 0;
        eventTable = new double[1 + noCPUs];
        eventTable[0] = jArr();
...
//            }
    ...
    public void jArrival() {
        ...
        //add to server (preempted)
        if (situation == 0) {
            for (int i = 0; i < config.numOfServers; i++) {
                if (eventTable[i + 1] - clock > e.serviceTime) {

                    Entity preEmpted = config.servers[i].release();
                    preEmpted.preEmpte(eventTable[i + 1]);
                    //add preempted
//                    int queueRange = 4;
                    /////////////////////////////////////
                    int situation2 = 0;
                    if (preEmpted.serviceTime <= queueRange) {
                        config.queues[0].addPreEmpted(preEmpted);
//                        situation2 = 1;
                    } else {
                        for (int j = 1; j <= (config.numOfQueues - 2); j++) {
                            if (preEmpted.serviceTime <= (queueRange * (j + 1))) {
                                config.queues[j].addPreEmpted(preEmpted);
                                situation2 = 1;
                                break;
                            }
                        }
                        if (situation2 == 0) {
                            config.queues[config.numOfQueues - 1].addPreEmpted(preEmpted);
                        }
                    }
                    /////////////////////////////////////
...
//        observer.report();
//        try {
            return observer.report2();
//        } catch (IOException ex) {
//            System.out.println("we have and io exception");
//        }
    }
...
//        int mio = 9;
        ...
        double nd = Math.ceil(delta * standardNormalDistribution + mio);
        if (nd < 1)
            return 1;
        return (int) nd;
