...
public class Polynomial {
    ...
    public Polynomial findIntegralPolynomial() {
        ...
        for (int i = 0; i < exps.size(); i++) {
            p.addExpresssion(exps.get(i).findIntegralExpression());
        }
        return p;
    }
    ...
    public double computeArea_DefiniteIntegral(double begin, double end) {
        double intEnd = 0;
        double intBegin = 0;
        intEnd += findIntegralPolynomial().computeValue(end);
        intBegin += findIntegralPolynomial().computeValue(begin);
        return intEnd - intBegin;
    }
    ...
    public double simpsonFormula(double a, double b) {
        return ((b - a) / 6) * (computeValue(a) + 4 * computeValue((a + b) / 2) + computeValue(b));
    }
    ...
    public double computeArea_Simpson(double begin, double end, double h) {
        int noOfInterval = (int) ((end - begin) / h) + 1;
        double sizeOfInterval = (end - begin) / noOfInterval;
        double sum = 0;
        for (int i = 0; i < noOfInterval; i++) {
            sum += simpsonFormula(begin + (i * sizeOfInterval), begin + ((i + 1) * sizeOfInterval));
        }
        return sum;
    }
    ...
    public double computeArea_MonteCarlo(double begin, double end, int n) {
        ...
        for (int i = 0; i < n; i++) {
            double x = begin + Math.random() * width;
            double y = Math.random() * height;
            if (computeValue(x) >= y)
                sumInArea++;
        }
        return (sumInArea / (double) n) * width * height;
